...
import TerminatePage from "../terminate-modal/page";
import DeclinePage from "../decline-modal/page"; 
...
export default function Modal({ isOpen, onClose, data }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-40 bg-black bg-opacity-50 flex items-center justify-center">
        <div className="w-[952px] h-[827px] py-[60px] px-[50px] gap-[20px] bg-white rounded-sm shadow-lg relative flex flex-col justify-between">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-4 mr-4">
              <Link href="/pages/edit-arena">
                <button className="items-center gap-[10px] opacity-100 px-[32px] py-[16px] rounded-[12px] border border-[#D4AF37] text-[#D4AF37] cursor-pointer font-aeonik font-bold text-[16px] leading-[100%] tracking-[0]">
                  Edit
                </button>
              </Link>

              <button
                onClick={() => setShowDeclineModal(true)} // <-- TRIGGER DECLINE MODAL
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-3 flex-grow">
            ...
            <DetailBox label="Entry" value={data.entry} />
      ...
      {/* Terminate Modal */}
      {showTerminateModal && (
        <TerminatePage onClose={() => setShowTerminateModal(false)} />
      )}

      {/* Decline Modal */}
      {showDeclineModal && (
        <DeclinePage onClose={() => setShowDeclineModal(false)} />
      )}
    </>
  );
}
